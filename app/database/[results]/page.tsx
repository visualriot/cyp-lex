...
//     setShowScrollToTop(false); // Immediately hide the button
//   };
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (isScrollingToTop) return; // Ignore scroll events during scrolling to top
  ...
  }, [loading, loadingMore, loadMoreRows]); // Consistent dependencies

  // scroll to top
  const scrollToTop = () => {
    setIsScrollingToTop(true); // Disable the scroll listener
    window.scrollTo({ top: 0, behavior: "smooth" });

    // Wait for the scroll animation to complete before hiding the button
    setTimeout(() => {
      setShowScrollToTop(false);
      setIsScrollingToTop(false); // Re-enable the scroll listener
    }, 500); // Adjust the timeout to match the scroll animation duration
  };
  ...
  return (
    <section className="results-page w-full flex flex-col gap-y-4 lg:gap-y-8">
      ...
      <button
        onClick={scrollToTop}
        className={`fixed bottom-8 right-8 bg-accent text-white w-16 h-16 rounded-full shadow-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300 smooth  ${showScrollToTop ? "opacity-100 visible" : "opacity-0 invisible"}`}
        aria-label="Scroll to top"
      >
        ↑
      </button>
